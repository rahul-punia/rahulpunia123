...
const request=https.request(options,function(res){
    res.on("data",function(data){
        // console.log(""+data);
        var json=JSON.parse(data);//Parsing means analyzing and converting a program into an internal format that a runtime environment can actually run, 
        //for example the JavaScript engine inside browsers. The browser parses HTML into a DOM tree. ... JavaScript is also downloaded, parsed, and then execute
        var user=json["url"];
       console.log(user);
     });
    res.on("end",function(){
