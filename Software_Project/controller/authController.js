    ...
    try{
     if(req.body.email && req.body.password){
      ...
      if(user.password==req.body.password){
       ...
       return res.status(200).json({//Here I changed status code from  401 t0 200 then alert visible on frontend
     ...
     throw new Error("Invalid Input");
      ...
      console.log("INValid Email or Password");
        return res.status(200).json({    //status codes are v.important
         status:"User can't be loggedIn",////Here I changed status code from  501 t0 200 then alert visible on frontend
         err
     });
    }
}
...
async function resetPassword(req,res){
  try{
   ...
   const user=await userModel.findOne({
    ...
    if(user){
     user.resetPasswordHandler(password,confirmPassword); //resetPasswordHandler function defined in userModel
     //save in db
    await user.save();
    res.status(200).json({
